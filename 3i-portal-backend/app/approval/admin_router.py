"""
3i Fund Portal — Admin Approval Verification Endpoints
Manages approval contacts and per-company verification settings.
...
async def update_contact(contact_id: int, request: ContactRequest, admin: UserInfo = Depends(require_admin)):
    """Update an approval contact's name and phone number."""
...
async def toggle_contact_include(contact_id: int, request: ContactIncludeRequest, admin: UserInfo = Depends(require_admin)):
    """Toggle whether a contact is included in SMS notifications."""
...
async def list_company_verifications(admin: UserInfo = Depends(require_admin)):
    """List all companies with their verification settings."""
...
async def set_company_verification(
    ...
    """Set whether a company requires purchase notice verification."""
