...
(function () {
    ...
    function elocTrimDecimals(v, digits) {
        if (v === null || v === undefined) return '';
        let s = Number(v).toFixed(digits);
        if (s.indexOf('.') !== -1) s = s.replace(/0+$/, '').replace(/\.$/, '');
        return s;
    }
    function elocPdfPrice(v) { return (v === null || v === undefined) ? '' : '$' + elocTrimDecimals(v, 4); }
    function elocPdfMult(v) { return elocTrimDecimals(v === null || v === undefined ? 1 : v, 4); }
    ...
    async function loadElocDetailsPanel(elocId) {
        ...
        html += elocRow('VWAP over Pricing Period', elocPdfPrice(d.vwap));
        html += elocRow('Total Volume over Pricing Period', elocPdfShares(d.aggregateVolume), { shaded: true });
        html += elocRow(`Low Price of Pricing Period<span class="pc-details-note">${lowTradeNote}</span>`, elocPdfPrice(d.lowPrice), { highlight: true });
        html += '</tbody></table>';

        html += elocSectionHeader('PURCHASE PRICE');
        html += `<div class="pc-details-formula">Purchase Price&nbsp;=&nbsp;max( Discount ${elocPdfMult(d.discountMultiplier)} × VWAP ${elocPdfPrice(d.vwap)} ,&nbsp;Low Price ${elocPdfPrice(d.lowPrice)} )&nbsp;=&nbsp;${elocPdfPrice(d.purchasePrice)}</div>`;
        html += '<table class="pc-details-table"><tbody>';
        html += elocRow(`Discount × VWAP&nbsp;(${elocPdfMult(d.discountMultiplier)} × ${elocPdfPrice(d.vwap)})`, elocPdfPrice(discountVwap));
        html += elocRow('Low Price', elocPdfPrice(d.lowPrice), { shaded: true });
        html += elocRow('Purchase Price (the greater of the two)', elocPdfPrice(d.purchasePrice), { highlight: true, bold: true });
        html += '</tbody></table>';
        ...
        html += `<div class="pc-details-total-box">${elocPdfShares(d.elocShares)} shares&nbsp;×&nbsp;${elocPdfPrice(d.purchasePrice)}&nbsp;=&nbsp;${elocPdfMoney(d.totalDollarAmount)}</div>`;
