// ─────────────────────────────────────────────────────────────────────────
// CloudFront Function — JWT validator for /position_risk_management/*
//
// Attached at the viewer-request stage of the /position_risk_management/*
// behavior on the 3ifundportal.com distribution (E15RA0YW10O89Y).
//
// Sibling of validate-data-management-jwt — same HS256 signing secret
// (portal's JWT_SECRET from /auth/login), same role=admin contract. The
// only differences are the path prefix and the WWW-Authenticate realm.
// A single Portal login issues a token that works in both apps.
//
...
//
// Logic:
//   1. Static files under /position_risk_management/* pass through.
//   2. Only /position_risk_management/api/* requires auth.
...
function deny(reason) {
    return {
        ...
        headers: {
            ...
            'www-authenticate': { value: 'Bearer realm="position-risk-management"' }
        },
...
function handler(event) {
    ...
    if (request.uri.indexOf('/position_risk_management/api/') !== 0) {
        return request;
    }
